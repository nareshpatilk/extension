...
let topology = {
  ...
  const updatedCountryMap = countryMaster.map((country) => ({
    ...country, // Spread the existing properties
  ...
  const mapGraph = (obj) => {
    const mapChart = Highcharts.mapChart("mapChart", {
      chart: {
        map: topology,
      },
    ...
    console.log("updatedCountryMap", updatedCountryMap);
  
    let countrArr = [];
    obj.forEach((c) => {
      let countryName = c.name;
      let countryFound = updatedCountryMap.find(
        (cname) =>
          cname.name
            .toString()
            .toLowerCase()
            .includes(countryName.toString().toLowerCase()) ||
          cname.iso2.toString().toLowerCase() ===
            countryName.toString().toLowerCase() ||
          cname.iso3.toString().toLowerCase() ===
            countryName.toString().toLowerCase()
      );
  ...
  const setChartData = (option) => {
    ...
    let obj = result[option].reduce((acc, curr) => {
      const currentDict = acc.find(
        (dict) =>
          dict.name.toString().trim().toLowerCase() ===
          curr.toString().trim().toLowerCase()
      );
      if (currentDict) {
        currentDict.y += 1;
      } else {
        acc.push({ name: curr.toString().trim(), y: 1 });
      }
      return acc;
    }, []);
  ...
  dynamicChart.addEventListener("click", dynamicChart);
